...
export default class City {
    ...
    static #columns = ['id', 'nome', 'codigo', null];

    // Colunas pesquisáveis pelo termo de busca
    static #searchable = ['nome', 'codigo'];
    ...
    static async insert(data) {
        ...
        if (!data.id_uf) {
            return { status: false, msg: 'Selecione uma unidade federativa', id: null, data: [] };
        }

        try {

            const clean = City.#sanitize(data);

            const [result] = await connection(City.table)
                .insert(clean)
                .returning('*');
    ...
    //Implementamos a pesquisa completa para a cidade
    static async find(data = {}) {
        const { term = '', limit = 10, offset = 0, orderType = 'asc', column = 0, draw = 1 } = data;
        //Total sem filtro
        const [{ count: total }] = await connection(City.table).count('id as count');
        //Monta WHERE da busca
        const search = term?.trim();
        function applySearch(query) {
            if (search) {
                query.where(function () {
                    for (const col of City.#searchable) {
        ...
        // Total filtrado
        const filteredQ = connection(City.table).count('id as count');
        applySearch(filteredQ);
        const [{ count: filtered }] = await filteredQ;
        // Dados paginados
        const orderColumn = City.#columns[column] || 'id';
        const orderDir = orderType === 'desc' ? 'desc' : 'asc';
        const dataQ = connection(City.table).select('*');
        applySearch(dataQ);
    ...
    //Implementamos a exclusão cidade
    static async delete(id) {
        if (!id) return { status: false, msg: 'ID é obrigatório' };

        try {
            await connection(City.table).where({ id }).del();
    ...
    static async update(id, data) {
        ...
        if (!data.id_uf) {
            return { status: false, msg: 'Selecione uma unidade federativa', data: [] };
        }
        ...
        try {
            ...
            if (!result) {
                return { status: false, msg: 'Cidade não encontrada', data: [] };
            }
    ...
    //Retorna apenas uma cidade pelo seu ID
    static async findById(id) {
        if (!id) return null;

        const row = await connection(City.table)
            .where({ id })
            .first();
