...
export default class Enterprise {
    ...
    static #columns = ['id', 'razao_social', 'nome_fantasia', 'cnpj', null];

    // Colunas pesquisáveis pelo termo de busca
    static #searchable = ['razao_social', 'nome_fantasia', 'cnpj'];

    //Insere uma nova empresa.
    static async insert(data) {

        if (!data.razao_social || data.razao_social.trim() === '') {
            return { status: false, msg: 'O campo razão social é obrigatório', id: null, data: [] };
        }
    ...
    //Implementamos a pesquisa completa para a empresa
    static async find(data = {}) {
        const { term = '', limit = 10, offset = 0, orderType = 'asc', column = 0, draw = 1 } = data;
        //Total sem filtro
        const [{ count: total }] = await connection(Enterprise.table).count('id as count');
    ...
    //Implementamos a exclusão empresa
    ...
    static async update(id, data) {
        ...
        if (!data.razao_social || data.razao_social.trim() === '') {
            return { status: false, msg: 'O campo razão social é obrigatório', data: [] };
        }
    ...
    //Retorna apenas uma empresa pelo seu ID
