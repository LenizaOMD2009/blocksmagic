...
export default class Contact {
    ...
    static #columns = ['id', 'nome', 'email', 'telefone', null];

    // Colunas pesquisáveis pelo termo de busca
    static #searchable = ['nome', 'email', 'telefone', 'celular'];
    ...
    static async insert(data) {
        ...
        if (!data.customer_id) {
            return { status: false, msg: 'Selecione um cliente', id: null, data: [] };
        }

        try {

            const clean = Contact.#sanitize(data);

            const [result] = await connection(Contact.table)
                .insert(clean)
                .returning('*');
    ...
    //Implementamos a pesquisa completa para o contato
    static async find(data = {}) {
        const { term = '', limit = 10, offset = 0, orderType = 'asc', column = 0, draw = 1 } = data;
        //Total sem filtro
        const [{ count: total }] = await connection(Contact.table).count('id as count');
        //Monta WHERE da busca
        const search = term?.trim();
        function applySearch(query) {
            if (search) {
                query.where(function () {
                    for (const col of Contact.#searchable) {
        ...
        // Total filtrado
        const filteredQ = connection(Contact.table).count('id as count');
        applySearch(filteredQ);
        const [{ count: filtered }] = await filteredQ;
        // Dados paginados
        const orderColumn = Contact.#columns[column] || 'id';
        const orderDir = orderType === 'desc' ? 'desc' : 'asc';
        const dataQ = connection(Contact.table).select('*');
        applySearch(dataQ);
    ...
    //Implementamos a exclusão contato
    static async delete(id) {
        if (!id) return { status: false, msg: 'ID é obrigatório' };

        try {
            await connection(Contact.table).where({ id }).del();
    ...
    static async update(id, data) {
        ...
        if (!data.customer_id) {
            return { status: false, msg: 'Selecione um cliente', data: [] };
        }
        ...
        try {
            ...
            if (!result) {
                return { status: false, msg: 'Contato não encontrado', data: [] };
            }
    ...
    //Retorna apenas um contato pelo seu ID
    static async findById(id) {
        if (!id) return null;

        const row = await connection(Contact.table)
            .where({ id })
            .first();
